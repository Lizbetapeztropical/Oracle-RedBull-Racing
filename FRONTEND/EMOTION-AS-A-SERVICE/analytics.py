...
def show_analytics():
    ...
    def load_data():
        ...
        # CORRECCIÓN 1: Crear YEAR de forma segura
        df['YEAR'] = pd.to_numeric(
            df['NAME_YEAR'].astype(str).str[:4], 
            errors='coerce'
        ).astype('Int64')

        # CORRECCIÓN 2: Limpiar columnas numéricas (evita errores futuros)
    ...
    if train_data.empty or test_data.empty:
        ...
        st.warning(
            f"⚠️ No hay suficientes datos para el año {year_choice}"
        )
    ...
    if not filtered_df.empty:
        ...
        input_data_driver = filtered_df[features].copy()

        # Limpiamos NaNs solo para predecir, pero mantenemos el dataframe original
        input_data_driver = input_data_driver.dropna()
    ...
    with col1:
        st.metric("Score", avg_score)

    with col2:
        st.metric("DNFs", last_dnf)

    with col3:
        st.metric("Pit Stops", total_stops)

    with col4:
        st.metric(
            f"Predicted Score ({model_choice})",
            predicted_avg_score
        )
    ...
    if not filtered_df.empty and 'Prediction' in filtered_df.columns:
        ...
        st.info(
            "No hay suficientes datos para este piloto."
        )
